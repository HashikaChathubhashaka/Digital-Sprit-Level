...
const byte HOLD_BTN = 5;
const byte SOUND_BTN = 6;
const byte MUTE_BTN = 7;



//variables for Holding the value
int decider = 0;
int R=0;
double holdValue = 0;     // Value to hold when HOLD button is pressed
...
// The math is a little different here compared to Adafruit's version to work 
VectorFloat QtoEulerAngle(Quaternion qt) {
  ...
  ret.y = atan2(2.0 * (qt.x * qt.y + qt.z * qt.w), (sqx - sqy - sqz + sqw));
  ret.x = asin(2.0 * (qt.x * qt.z - qt.y * qt.w) / (sqx + sqy + sqz + sqw));    
...
void setup() {
  ...
  Serial.begin(9600); // Only needed for Debug otherwise can comment out
...
bool sound_ans=false;
int x=0;

void Sound_on(){

  if (x==0){
      if (digitalRead(SOUND_BTN) == LOW){
        sound_ans = true;
        x=1;
      }
...
void formatDisplay(double angleVal, byte dispRotate) {
  ...
  if (round(abs(angleVal)) <= 1) {
    ...
    if (dispRotate == 0 || dispRotate == 2) { 
      ...
      //sound on
      if(sound_ans){
        tone(buzzer, 950);  // Set the frequency to 1000 Hz
        delay(250);            // Play for 1 second
        Sound_on();
 ...
 // double angVal = 0;
  ...
  if (mpu.dmpGetCurrentFIFOPacket(fifoBuffer)) {
    ...
    */
    ...
    if (ea.y > 0 && ea.x > 35 && ea.x <= 90) {
      angVal = (90 - ea.x);
      dispRotate = 0;
    }
    // Angling right side up RIGHT is TOP (toward PLUMB)
    if (ea.y > 0 && ea.x <= 35 && ea.x > -35) {
      angVal = ea.x;
      dispRotate = 1;
    }
    // LEFT IS TOP (PLUMB)
    ...
    if (ea.y < 5 && ea.y > -20 && ea.x <= 35 && ea.x > -35 && ea.z < 5) {
      angVal = 90 - ea.x;
      if (angVal > 50) {
    ...
    holdValue = angVal; // Store the current angle value
    R= dispRotate;
